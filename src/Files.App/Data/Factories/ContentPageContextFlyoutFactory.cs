	...
	public static class ContentPageContextFlyoutFactory
	{
			...
			ShellViewModel? itemViewModel = null)
		{
			...
			return new List<ContextMenuFlyoutItemViewModel>()
			{
				...
				{
					IsVisible = VxFilesEnvironment.SupportsPackageIdentity && selectedItems.All(x => (x.PrimaryItemAttribute == StorageItemTypes.Folder || x.IsExecutable || (x is IShortcutItem shortcutItem && FileExtensionHelpers.IsExecutableFile(shortcutItem.TargetPath))) && !x.IsArchive && !x.IsItemPinnedToStart) && UserSettingsService.GeneralSettingsService.ShowPinToStart,
					ShowOnShift = true,
				}.Build(),
